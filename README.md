...
- [Embedded/Local Skills](#embeddedlocal-skills)
  - [embed](#craftdesk-embed-name-options)
  - [unembed](#craftdesk-unembed-name-options)
...
- ✅ **Team-shared** - Distributed via `git clone` (not `craftdesk install`)
- ✅ **Auto-synced** - Sync to other AI assistants with multi-agent support
- ⚠️ **Not managed** - No version updates via `craftdesk update`
...
**Options:**
- `--type <type>` - Craft type: skill, agent, command, hook (default: skill)
- `--skip-sync` - Skip multi-agent sync after embedding
...
**What happens:**
1. ✅ Adds skill to `craftdesk.json` `embedded` array
2. ✅ Updates `.claude/skills/.gitignore` to allow the skill
3. ✅ Syncs to other AI assistants (if multi-agent enabled)

**Requirements:**
- Skill directory must exist (`.claude/skills/my-local-skill/`)
- Cannot embed a managed dependency (remove from `dependencies` first)
- Cannot embed the same skill twice
...
**What happens:**
1. ✅ Removes skill from `craftdesk.json` `embedded` array
2. ✅ Updates `.claude/skills/.gitignore` (skill becomes orphaned)
3. ⚠️ Skill files remain unless `--remove` is used

**After unembedding:**
- Skill becomes "orphaned" (commented in `.gitignore`)
- Manually delete files or re-embed it
- Or add it to `dependencies` to manage it
...
```bash
# You have: ruby-expert@1.0.0 (from registry)
...
# 📚 Skills:
#   ruby-expert@1.0.0
#   postgres-expert@2.0.0
#   my-project-skill@1.0.0 📌 (embedded)
